...
Sanity check on the harness: our reproduction of the base Kriti model scores 40.51 on NepTel against its published 40.6, a match to rounding. The instrument is sound before any other number is trusted.
...
Kriti Telephony cuts telephony WER from 40.6 to 32.4 while keeping general Nepali strong at 6.3, close to the base model's 4.1. It is a telephony-tuned model that stays capable on clean audio.
...
| Data | Encoder | NepTel WER | vs base (40.51) |
|---|---|---:|---|
...
|---|---|---:|
| `kriti-baseline.neptel.json` | base Kriti (published output) | 40.59 |
| `kriti-baseline-redecode.neptel.json` | base Kriti (our re-decode) | 40.51 |
| `nepaliconformer.neptel.json` | NepaliConformer offline | 33.81 |
| `kriti-telephony.neptel.json` | **Kriti Telephony** | **32.38** |

```bash
python eval/score.py --hyp benchmark/outputs/kriti-telephony.neptel.json   # 32.38
```
